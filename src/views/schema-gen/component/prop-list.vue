...
<!--      <el-button type="primary" link size="small" @click="onLogClick">log</el-button>-->
    </template>
    ...
    <div>
      <div v-for="(ip, index) in propList" :key="index"
        ...
        <div class="opt-group" v-if="innerCurrentProp?.key === ip.key">
          <el-button type="primary" link size="small" @click.stop="onUpClick(index)" v-if="index > 0">上移</el-button>
          <el-button type="primary" link size="small" @click.stop="onDownClick(index)" v-if="index < propList.length - 1">下移
          </el-button>
          <el-button type="primary" link size="small" @click.stop="onDeleteClick(index)">删除</el-button>
          <el-button type="primary" link size="small" @click.stop="onCopyClick(index)">复制</el-button>
        </div>
      </div>
    </div>
  </block>
